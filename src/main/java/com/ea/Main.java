...
public class Main {
    ...
    private static void inspectDetached(DevTools chromeDevTools) {
    }
    ...
    private static void enableNetworkOffline(DevTools devTools) {
        ...
        devTools.send(emulateNetworkConditions(true, 100, 1000, 2000, Optional.of(ConnectionType.cellular3g)));

        devTools.addListener(loadingFailed(),
                loadingFailed -> assertEquals(loadingFailed.getErrorText(), "net::ERR_INTERNET_DISCONNECTED"));
    }
    ...
    private static void enableNetworkOnline(DevTools devTools) {
        ...
        devTools.send(emulateNetworkConditions(false, 100, 5000, 2000, Optional.of(ConnectionType.cellular4g)));
    ...
    private static void Selenium4MiscFetures(ChromeDriver chromeDriver) {
        ...
        WebElement checkbox = chromeDriver
                .findElement(withTagName("input").below(By.name("Male")).toLeftOf(By.name("Hindi")));
        checkbox.click();
        System.out.println(checkbox.getAttribute("name"));

        WebElement txtIntial = chromeDriver
                .findElement(withTagName("input").below(By.id("TitleId")).above(By.id("FirstName")));

        txtIntial.sendKeys("KK");

        // list of elements
        List<WebElement> lstElements = chromeDriver.findElements(
                withTagName("input").below(By.xpath("//h2[text()=' User Form ']")).above(By.name("Save")));

        Var elements = (Var) lstElements.stream().map(x -> x.getAttribute("input"));
